...
export interface StageOptions {
  /** Build the book (and any extras) against the provided scene. */
  make: (scene: THREE.Scene) => StageBuild
...
export function useBookStage(
  ...
  useEffect(() => {
    ...
    const sky = (() => {
      ...
      ctx2d.fillStyle = g; ctx2d.fillRect(0, 0, 8, 256)
    ...
    let built = makeRef.current(scene)
